...
class CategoriesCPresenter extends BasePresenter
{
    private $parent;
	private $edited;
	private $type=3;
	...
	public function startup(){
		parent::startup();
		$this->addBreadcrumbs("Kategorie vozového parku", $this->link(":Admin:CategoriesC:default"));
	}
	...
	public function actionDefault($parent=null){
		...
		if($parent<>null){
			...
			$this->addBreadcrumbs("Podkategorie ".$this->template->parent->name, $this->link(":Admin:CategoriesC:default", array('parent', $parent)));
		}
	...
	public function actionAdd($parent=null){
		...
		$this->parent = $parent;
		$this->addBreadcrumbs("Přidání kategorie", $this->link(":Admin:CategoriesC:add", $parent));
	}
	...
	public function actionEdit($id, $parent){
		...
		$this->setView("addEdit");
		$this->addBreadcrumbs("Úprava kategorie ".$pageDetails->name, $this->link(":Admin:CategoriesC:edit", [$id, $parent]));
	}

	public function actionDelete($id){
		$this->categoryManager->delete($id);
		$this->redirect(":Admin:CategoriesC:default");
		/*
		try{
			if(){
		...
		if($form->isValid()){
			try{
	            ...
	            */
				...
				$this->redirect(":Admin:CategoriesC:default", $this->parent);
