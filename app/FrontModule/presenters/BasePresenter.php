...
abstract class BasePresenter extends Nette\Application\UI\Presenter
{
	...
	public $contactCategories = array(50=>"Vedení společnosti", 40=>"Dispečink", 30=>"Administrativa", 10=>"Řidiči", 20=>"Strojníci");
	...
	public function startup(){
        ...
        $this->template->categories = $this->categoryManager->get()->where("active = ?", true)->where("parent IS NULL");
        ...
        */
        ...
        $disabledDaysArr = array();
        foreach(explode("\n", $this->settings->disabledDays) as $day){
			$disabledDaysArr[] = "'".$day."'";
        }
        $this->template->datesDisabled = "[".(implode(",", $disabledDaysArr))."]";
    ...
    public function beforeRender(){
        ...
        $this->template->addFilter('webpImage', function($image, $width=null, $height=null, $title, $method = "EXACT") use ($presenter){
            return $presenter->webpImage($image, $width, $height, $title, $method);
        });
    }
    ...
    */
    ...
    public function webpThumb($image, $width = null, $height = null, $method = "EXACT")
    {
       $methods = array(
           "FIT"=>0,
           "FILL"=>4,
       ...
       if(($width==null || $height==null) && $method=="EXACT"){
        ...
        if ($width == null && $height == null) {
        	...
        	$fileSize = filesize($uploadDir . DIRECTORY_SEPARATOR . $imagePath);
        	$thumbImagePath = md5($imagePath . $fileSize) . '.' . "webp";
            $image->save($thumbPath . DIRECTORY_SEPARATOR . $thumbImagePath, 100, \Nette\Utils\Image::WEBP);
            return $thumbDir . "/" . $thumbImagePath;
        }
        ...
        $thumbImagePath = md5($width . $height . $imagePath . $fileSize) . '.' . "webp";
    ...
    public function webpImage($image, $width = null, $height = null, $title='', $method = "EXACT")
    {
		$origFormat = $this->thumb($image, $width = null, $height = null, $method = "EXACT");
		$webpFormat = $this->webpThumb($image, $width = null, $height = null, $method = "EXACT");
		return Html::el("picture")
			->insert(0, Html::el("source")
				->srcset($webpFormat)
				->type("image/webp")
			)
			->insert(1, Html::el("img")
				->src($origFormat)
				->title($title)
			);
		
		//return '<picture><source srcset="'.$webpFormat.'" type="image/webp"><img src="https'.$origFormat.'" alt="'.$title.'"></picture>';
	}
    ...
    public function recalculateBasket($basket = "basket"){
		...
		if(!empty($this->$basket->containers)){
			foreach($this->$basket->containers as $container){
					...
					if($prod){
						foreach(explode("|", $prod->attributes) as $aKey){
							...
							if($key==$weightAttr){
								...
								$totalWeight = max($totalWeight, (int)$aVal->name);
							}
							if($key==$volumeAttr){
								$aVal = $this->attributeManager->findValue($val);
								$totalVolume = max($totalVolume, (int)$aVal->name);
							}
						}
					}
				}
				...
				else if(!empty($this->$basket->termFrom)){
				...
				foreach($this->$basket->materials as $material){
					if(!empty($material->priceObj)){
						...
						if($material->product == $this->settings->betonProduct){
							$standardBetonPrice = $material->priceObj->priceFrom * $material->amount;
							if($isBetonAdd){
		...
		if($materialTons>0 && $materialTons<=6){
			...
			if(!empty($this->$basket->containers[0]->product)){
				$deliveryPriceStriked = $deliveryPrice;
				$deliveryPrice -= $this->settings->smallDelivery;
			}
		}
		...
		$betonAddPrice = 0;
		if($standardBetonPrice > 0){
			if($basket=="basket" && count($this->$basket->containers)==1 && !empty($this->$basket->containers[0]->term)){
				$dateTime = \Nette\Utils\DateTime::createFromFormat("d/m/Y", $this->$basket->containers[0]->term);
				$this->$basket->termFrom = $this->$basket->containers[0]->term;
			}
			else{
				$dateTime = \Nette\Utils\DateTime::createFromFormat("d/m/Y", $this->$basket->termFrom);
			}
			if(!empty($dateTime)){
				if(array_search($dateTime->format("d.m."), $holidays)!==false){
					$betonAddPrice = $standardBetonPrice * 0.1;
				}
				else if($dateTime->format("N")>=6){
					$betonAddPrice = $standardBetonPrice * 0.1;
				}
        ...
        $this->$basket->betonAddPrice = $betonAddPrice;
        $this->$basket->price = $totalPrice + $weekendPrice + $deliveryPrice + $betonAddPrice;
        ...
        $this->$basket->deliveryPriceStriked = $deliveryPriceStriked;
