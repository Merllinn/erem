...
class PagesPresenter extends BasePresenter
{
	...
	public function handleLiquidate($id, $val = true){
		try{
			$this->pageManager->update(array("liquidate"=>$val), $id);
			$this->redirect("this");
		}
		catch(DibiDriverException $e){
			$this->flashMessage($e->getMessage());
    ...
    public function createComponentPages($name)
    {
        ...
        if($presenter->parent==39){
	        $grid->addColumnText('liquidate', 'Likvidujeme')
	            ->setRenderer(function($row) use ($presenter) {
	                if($row->liquidate){
						if($this->user->identity->role==9){
                    		return Html::el("a")->class("")->href($presenter->link("liquidate!", $row->id, false))->setText("ANO");
						}
						else{
                    		return "ANO";
						}
						...
						if($this->user->identity->role==9){
                    		return Html::el("a")->class("")->href($presenter->link("liquidate!", $row->id))->setText("NE");
						}
						else{
                    		return "NE";
						}
