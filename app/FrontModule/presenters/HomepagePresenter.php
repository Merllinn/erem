...
final class HomepagePresenter extends HomepageForms
{
    ...
    public function createComponentOrderForm(){
        ...
        //$form["payment"]->addRule($form::FILLED, "Je nutné potvrdit způsob platby. Platba je možná pouze hotově řidiči.");
        //$form ->addCheckbox("agree", "Souhlasím s obchodními podmínkami *")
        $agreeText = Html::el(null)
        	->addText("Souhlasím s ")
        	->addHtml(Html::el('a')
        		->href($this->link(":Front:Homepage:page", 'obchodni-podminky'))
        		->target("_blank")
        		->setText('obchodními podmínkami'))
        	->addText(" *");
        $form ->addCheckbox("agree", $agreeText)
        	->getControlPrototype()->class("form-check-input");
        //$form["agree"]->addRule($form::FILLED, "Je nutné potvrdit souhlas s obchodními podmínkami.");
        ...
        $form->addSelect("service", "Mám zájem o", $services)
        	->setPrompt("vyberte");

        $form->addText("locality", "Lokalita černé skládky")
        	->getControlPrototype()->class("form-control");
    ...
    */
    public function saveOrder(Form $form){
        ...
        if($values->type==8 && empty($values->service)){
			$form->addError("Vyberte službu o kterou máte zájem.");
        }
        if($values->type==7 && empty($values->locality)){
			$form->addError("Zadejte lokalitu skládky.");
        }
        if($basketfield != "basketD" && $values->payment != true){
			$form->addError("Je nutné potvrdit způsob platby. Platba je možná pouze hotově řidiči.");
        }
        if(/*$basketfield != "basketD" && */$values->agree != true && $values->type != 8 && $values->type != 7){
			$form->addError("Je nutné potvrdit souhlas s obchodními podmínkami.");
        }
            ...
            try{
				...
				*/
				...
				if($values->type==8 || $values->type==7){
					...
					$this->sendMailFromTemplate("demandConfirmEshop2.latte", $data, $this->settings->email, "Nová poptávka");
					$this->sendMailFromTemplate("demandConfirmEshop2.latte", $data, $values->email, "Nová poptávka");
				}
