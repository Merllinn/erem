...
final class ProductManager
{
    ...
    {
        return $this->database->query("select p.*, i.file from ".self::BASE." p left join ".self::IMAGES." i on i.products_id = p.id and i.main=1 WHERE p.onHome=1 ORDER BY `order`");
    }

    public function findByCategory($category)
    {
        return $this->database->query("select p.*, i.file from ".self::BASE." p left join ".self::IMAGES." i on i.products_id = p.id and i.main=1 where categories like ? ORDER BY `order`", "%|".$category."|%");
    }
    ...
    public function getSidePhotos($product){
        return $this->getPh()
            ->where("products_id", $product)
            ->where("main = ?", 0)
            ;
    ...
    public function getOrderPhoto($product){
        return $this->getPh()
            ->where("products_id", $product)
            ->where("toOrder = ?", 1)
            ->fetch();
