...
class ProductGalleryPresenter extends BasePresenter
{
    ...
    */
    ...
    public function handleSetToOrder($id){
        $main = $this->productManager->getOrderPhoto($this->product);
        if($main)
            $this->productManager->updatePhoto(array("toOrder"=>0),$main->id);
        $this->productManager->updatePhoto(array("toOrder"=>1),$id);
        $this->redirect("this");
    }
